...
from app.schemas.github_commit import CommitReadmeRequest, CommitReadmeResponse
...
authorization: Optional Bearer token from Authorization header
    ...
    logger.info(
        f"[README Generation] Raw github_url: '{github_url}' (length: {len(github_url)})"
    )
    logger.info(
        f"[README Generation] Raw github_api_key: {'Provided' if github_api_key else 'Not provided'} (length: {len(github_api_key) if github_api_key else 0})"
    )

    if github_api_key:
        logger.info(
            f"[README Generation] GitHub API key provided (for private repo access)"
        )
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"[URL Validation] Valid URL - Owner: '{owner}' (len={len(owner)}), Repo: '{repo_name}' (len={len(repo_name)})"
            )
        ...
        try:
            logger.info(
                f"[Access Check] Checking if {owner}/{repo_name} is accessible..."
            )
            if github_api_key:
            ...
            is_accessible, repo_data, is_public = await is_repository_accessible(
                github_url, github_api_key
            )
            logger.info(
                f"[Access Check] Repository {owner}/{repo_name} is {'PUBLIC' if is_public else 'PRIVATE'} and accessible"
            )
        ...
        status_str = (
            ...
            if readme_record.status
            else ReadmeStatus.PENDING.value
        )
        return GenerateReadmeResponse(id=readme_record.id, status=status_str)
...
async def upload_zip_readme(
    ...
    try:
        ...
        logger.info(
            f"[ZIP Upload] File size: {file_size} bytes ({file_size / (1024 * 1024):.2f} MB)"
        )
        ...
        try:
            ...
            logger.info(
                f"[ZIP Upload] Saved ZIP file to {zip_path} ({file_size} bytes)"
            )
        except Exception as e:
        ...
        logger.debug(
            "[ZIP Upload] Step 5: Getting or creating session (database operation)"
        )
        try:
            ...
            logger.error(
                f"[ZIP Upload] Database error when getting/creating session: {str(e)}"
            )
            logger.error(f"[ZIP Upload] Error type: {type(e).__name__}")
            logger.error(f"[ZIP Upload] Traceback: {traceback.format_exc()}")
            ...
            if "connection refused" in error_str or "errno 61" in error_str:
                logger.error(
                    "[ZIP Upload] Database connection refused - PostgreSQL may not be running"
                )
                raise HTTPException(
        ...
        logger.debug(
            "[ZIP Upload] Step 6: Creating GeneratedReadme record (database operation)"
        )
        try:
            readme_record = GeneratedReadme(
                session_id=session.id,
            ...
            logger.info(
                f"[ZIP Upload] Created record {readme_record.id} with PENDING status"
            )
        except Exception as e:
        ...
        asyncio.create_task(
            process_zip_readme_generation_async(readme_record.id, zip_path)
        )
        ...
        if zip_path and os.path.exists(zip_path):
            try:
                ...
                logger.warning(
                    f"[ZIP Upload] Error during cleanup: {str(cleanup_error)}"
                )
...
async def get_readme(readme_uuid: UUID, db: AsyncSession = Depends(get_db)):
    """
...
async def download_readme(
    ...
    if status_value == ReadmeStatus.COMPLETED.value:
        ...
        try:
            ...
            return FileResponse(
                tmp_path, media_type="text/markdown", filename="README.md"
            )
        except Exception as e:
        ...
        status_value == ReadmeStatus.PENDING.value
        or status_value == ReadmeStatus.PROCESSING.value
    ):
        # Still processing - return 202 Accepted
        logger.debug(f"[Download] README {readme_uuid} status: {status_value}")
        response_data = DownloadReadmeResponse(status=status_value, readme_content=None)
        return JSONResponse(
            content=response_data.model_dump(), status_code=status.HTTP_202_ACCEPTED
        )
...
async def update_readme(readme_uuid: UUID, db: AsyncSession = Depends(get_db)):
    """
    ...
    logger.info(
        f"[Update README] Successfully updated was_downloaded for {readme_uuid}"
    )

    return {"message": "README marked as downloaded", "id": str(readme_uuid)}


@router.post("/commit", response_model=CommitReadmeResponse)
async def commit_readme_to_repository(
    request: CommitReadmeRequest, db: AsyncSession = Depends(get_db)
):
    """
    Commits generated README.md directly to GitHub repository
    """
    # 1. Query User
    ...
    # 2. Query GeneratedReadme
    readme_result = await db.execute(
        select(GeneratedReadme).where(GeneratedReadme.id == request.readme_id)
    )
    readme = readme_result.scalar_one_or_none()
    if not readme:
        raise HTTPException(status_code=404, detail="README not found")

    # 3. Validate ownership
    if readme.user_id != request.user_id:
        raise HTTPException(status_code=403, detail="User does not own this README")

    # 4. Validate input_method
    if readme.input_method != InputMethod.GITHUB_AUTH:
        raise HTTPException(
            status_code=403, detail="Can only commit READMEs generated via GitHub OAuth"
        )

    # 5. Validate repo_url and branch
    if not readme.repo_url or not readme.branch:
        raise HTTPException(
            status_code=400, detail="Repository URL or branch is missing"
        )

    # 6. Parse owner and repo
    try:
        parts = readme.repo_url.rstrip("/").split("/")
        owner = parts[-2]
        repo = parts[-1]
    except IndexError:
        raise HTTPException(status_code=400, detail="Invalid repository URL format")

    # 7. Get branch from readme
    branch = readme.branch

    # 8. Decrypt token
    if not user.github_token:
        raise HTTPException(status_code=401, detail="GitHub not connected")
    decrypted_token = decrypt_token(user.github_token)
    if not decrypted_token:
        raise HTTPException(status_code=500, detail="Token decryption failed")

    # 9. Build commit message
    if request.commit_message:
        message = request.commit_message.strip()
    else:
        message = "docs: Add AI-generated README"
    if request.extended_description:
        message = f"{message}\n\n{request.extended_description.strip()}"

    # 10. Get README content
    if not readme.readme_content:
        raise HTTPException(status_code=400, detail="README content is empty")
    content = readme.readme_content

    # 11-13. Check if file exists and create/update
    try:
        existing_sha = await get_file_sha(
            owner, repo, "README.md", branch, decrypted_token
        )

        if existing_sha:
            # Update existing file
            result = await update_file_in_repo(
                owner,
                repo,
                "README.md",
                content,
            ...
            # Create new file
            result = await create_file_in_repo(
                owner, repo, "README.md", content, branch, message, decrypted_token
            )
        ...
        if status_code == 401:
            ...
            raise HTTPException(
                status_code=403, detail="No permission to push to this repository"
            )
            ...
            raise HTTPException(
                status_code=409, detail="Commit conflict, branch was modified"
            )
        else:
            raise HTTPException(status_code=502, detail=f"GitHub API error: {str(e)}")
    except httpx.RequestError:
        raise HTTPException(status_code=500, detail="GitHub API unavailable")

    # 14-15. Update database record
    readme.was_committed = True
    readme.commit_url = commit_url
    readme.commit_sha = commit_sha
    readme.updated_at = datetime.utcnow()
    await db.commit()
    await db.refresh(readme)

    # 16. Return response
    return CommitReadmeResponse(
        success=True,
