      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step) => (
            <div
              key={step.number}
              className="flex flex-col items-center text-center max-w-[420px] mx-auto px-4"
            >
              <div className="w-12 h-12 rounded-full bg-green-500 text-white flex items-center justify-center text-xl font-bold mb-4">
                {step.number}
              </div>
              <h3 className="font-semibold text-lg text-green-200 mb-2 max-w-[160px]">
                {step.title}
              </h3>
              <p className="text-gray-600 text-sm text-white mb-10 md:max-w-[160px] max-w-[300px]">
                {step.description}
              </p>
            </div>
          ))}
