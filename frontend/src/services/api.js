...
export const pollReadmeStatus = async (
  readmeId,
  maxAttempts = 60,
  intervalMs = 2000,
) => {
...
export const initiateGitHubOAuth = async () => {
  try {
    ...
    const errorMessage =
      error.response?.data?.detail || "Failed to initiate GitHub OAuth";
...
export const getAuthStatus = async (token) => {
  try {
    ...
    const errorMessage =
      error.response?.data?.detail || "Failed to check authentication status";
    throw new Error(errorMessage);
  }
};
...
export const commitReadme = async (
  ...
  try {
    ...
    const errorMessage =
      error.response?.data?.detail || "Failed to commit README to GitHub";
    throw new Error(errorMessage);
  }
};
// ============================================
...
export const getRepositoryBranches = async (owner, repo) => {
  try {
    ...
    // Extract error message from backend response
    const errorMessage =
      error.response?.data?.detail || "Failed to fetch branches";
    throw new Error(errorMessage);
  }
};
